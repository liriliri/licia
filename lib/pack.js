...
function genPackage(pkgName, files) 
{
    util.mkdir('packages/' + pkgName, function ()
    {
        ...
        fs.writeFile(path.resolve('packages/' + pkgName + '/package.json'), JSON.stringify(packInfo, null, 2), 'utf-8', function (err) 
        {
            if (err) console.log(err);
        });
    });
    ...
    util.each(files, function (file)
    {
        ...
        ret.push(function (cb)
        {
            fs.readFile(file, 'utf-8', function (err, data)
            {
                ...
                {
                    fs.writeFile(path.resolve('./packages/' + pkgName, modName + '.js'), data, 'utf-8', function (err)
                    {
                        if (err) return cb(err);

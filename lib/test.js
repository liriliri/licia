...
function collectTestScripts(data) {
    ...
    if (scripts.before) {
        testScripts.before.push(scripts.before);
    }
}

async function runTestBeforeScripts() {
    const before = testScripts.before;
    for (let i = 0, len = before.length; i < len; i++) {
        await runScript(before[i]);
    }
}
...
async function genTestUtil(modName, { extension = ['js'] } = {}) {
    const outputDir = options.demo ? '.licia/demo' : '.licia/testUtil';
    let format = 'global';
    ...
    map.sources = map.sources.map(source => {
        ...
        source = 'src/' + lowerCase(source[0]) + '/' + source;

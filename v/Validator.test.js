...
it('default plugins', function ()
{
    ...
    expect(validator.validate({testNum: ''})).to.equal('testNum should be a number');

    expect(validator.validate({testStr: ''})).to.be.true;
    expect(validator.validate({testStr: 5})).to.equal('testStr should be a string');

    expect(validator.validate({testBool: true})).to.be.true;
    expect(validator.validate({testBool: ''})).to.equal('testBool should be a boolean');

    expect(validator.validate({testRegExp: '1234'})).to.be.true;
    expect(validator.validate({testRegExp: 't'})).to.equal('testRegExp should match given regexp /\\d+/g');
});
...
it('add plugin', function ()
{
    ...
    {
        if (config && typeof val !== 'number') return key + ' should be a number';
    ...
    expect(validator.validate({test: 'a'})).to.equal('test should be a number');
